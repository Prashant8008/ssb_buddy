...
def create_post(author_id: int, author_username: str, author_avatar: str,
    ...
    try:
        ...
        import logging
        import uuid
        logging.error(f"Failed to create post in MongoDB: {e}")
        doc["id"] = f"fallback-{uuid.uuid4()}"
        doc.pop("_id", None)
        return doc
...
def get_feed(
    ...
    try:
        ...
        import logging
        logging.error(f"Failed to fetch posts from MongoDB: {e}")
        return []
...
def get_post(post_id: str) -> dict | None:
    try:
        ...
        import logging
        logging.error(f"Failed to get post {post_id} from MongoDB: {e}")
        return None
...
def update_post(post_id: str, author_id: int, data: dict) -> dict | None:
    ...
    try:
        ...
        import logging
        logging.error(f"Failed to update post {post_id} in MongoDB: {e}")
        return None
...
def delete_post(post_id: str, author_id: int) -> bool:
    try:
        ...
        import logging
        logging.error(f"Failed to delete post {post_id} in MongoDB: {e}")
        return False
...
def toggle_like(post_id: str, user_id: int) -> dict:
    ...
    try:
        ...
        import logging
        logging.error(f"Failed to toggle like on post {post_id} in MongoDB: {e}")
...
def toggle_save(post_id: str, user_id: int) -> dict:
    try:
        ...
        import logging
        logging.error(f"Failed to toggle save on post {post_id} in MongoDB: {e}")
        if isinstance(e, ValueError):
            raise e
        return {"saved": False}
...
def add_comment(post_id: str, author_id: int, author_username: str,
    ...
    try:
        ...
        import logging
        import uuid
        logging.error(f"Failed to add comment to post {post_id} in MongoDB: {e}")
        doc["id"] = f"fallback-{uuid.uuid4()}"
        doc.pop("_id", None)
        return doc
...
def get_comments(post_id: str) -> list[dict]:
    try:
        ...
        import logging
        logging.error(f"Failed to get comments for post {post_id} from MongoDB: {e}")
        return []
...
def delete_comment(comment_id: str, author_id: int, post_id: str) -> bool:
    ...
    try:
        ...
        import logging
        logging.error(f"Failed to delete comment {comment_id} in MongoDB: {e}")
        return False
...
def count_posts(
    ...
    try:
        ...
        import logging
        logging.error(f"Failed to count posts in MongoDB: {e}")
        return 0

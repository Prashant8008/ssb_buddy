        ...
        <div className="flex-1 max-w-md mx-8 hidden lg:block">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-navy-800 border-none rounded-full py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-gold-500 transition-all"
            />
          </div>
        </div>

        <div className="flex items-center gap-1 md:gap-4">
          <NavIcon icon={<Home size={20} />} label="Feed" to="/" />
          ...
          <NavLink to="/profile" className="w-8 h-8 rounded-full bg-army-500 border-2 border-gold-500 overflow-hidden">
...
const NavIcon = ({ icon, label, to }: { icon: React.ReactNode, label: string, to: string }) => (
  <NavLink 
    ...
    className={({ isActive }) => `
      ...
      ${isActive ? 'text-gold-500 bg-navy-800' : 'text-navy-300 hover:text-white hover:bg-navy-800'}
    `}
  >
    {icon}
